...
import (
	...
	"encoder-project/framework/queue"
	"encoding/json"
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error {
	if jobResult.Job.ID != "" {
		log.Printf("Message ID: %v. Error with job: %v with video: %v",
			jobResult.Message.DeliveryTag, jobResult.Job.ID, jobResult.Error.Error())
	} else {
		log.Printf("Message ID: %v. Error parsing message: %v",
